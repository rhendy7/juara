...
class Auth extends MX_Controller
{
		...
		if ($this->tank_auth->is_logged_in()) {									// logged in
			...
			if ($this->form_validation->run()) {								// validation ok
				if ($this->tank_auth->login(
					...
					redirect('redirection');							//load admin page
	...
	function add_pengelola()
	{
		if (!$this->config->item('allow_registration', 'tank_auth')) {	// registration is off
			...
			//custom
			$this->form_validation->set_rules('id_unit_kerja', 'id_unit_kerja');
			...
			if ($this->form_validation->run()) {								// validation ok
				if (!is_null($data = $this->tank_auth->add_pengelola(
						...
						$this->form_validation->set_value('id_unit_kerja')))) {									// success
			...
			$data['id_unit_kerja'] = $this->uri->segment(3);
			$data['use_username'] = $use_username;
			$data['captcha_registration'] = $captcha_registration;
			$data['use_recaptcha'] = $use_recaptcha;
