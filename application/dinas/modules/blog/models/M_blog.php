...
class M_blog extends CI_Model {
	...
	function tambah_berita($form_data)
	{
		
		$form_data['tanggal_input'] = date('Y-m-d h:m:s');
			
			
		if($this->db->insert('b_berita', $form_data)==TRUE) {
			return TRUE;
	...
	function get_news_pagination($id_unit_kerja, $id_kat,$num, $offset)
	{
		if($num=="") {
		$num=0;
		}
	 
		$sql = "SELECT b.*, k.nama AS kategori FROM b_berita b, b_kategori_berita k WHERE b.id_kategori = k.id AND b.id_kategori = $id_kat AND b.id_unit_kerja = $id_unit_kerja ORDER BY b.id DESC LIMIT $num, $offset ";
